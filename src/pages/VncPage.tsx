...
export function VncPage() {
  ...
  const vncSettings = useSettingsStore((s) => s.settings.vnc);
  const openLog = useConnectionLogsStore((s) => s.openLog);
  const closeLog = useConnectionLogsStore((s) => s.closeLog);

  const [launchInfo, setLaunchInfo] = useState<VncPageLaunchInfo | null>(null);
  ...
  useEffect(() => {
    ...
    const ensureStandaloneSession = async () => {
      ...
      ensureSession({
        ...
        hostLabel: bootstrap.hostLabel ?? "VNC",
  ...
  const connect = useCallback(async () => {
    ...
    try {
      const result = await invoke<VncLaunchResult>("vnc_connect", {
        ...
        password,
        options: {
          preferredLinuxClient: vncSettings.linuxClient,
          fullscreen: vncSettings.fullscreen,
      ...
      notify(APP_NAME, t("notifications.vncConnected", { host: tab.hostLabel }));
    } catch (err) {
  ...
  }, [closeLog, host, openLog, password, sessionConnection, setLastConnected, t, tab, tabId, targetHost, targetPort, updateTabStatus, vncSettings.fullscreen, vncSettings.linuxClient, vncSettings.viewOnly]);
  ...
  if (
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-sm text-[var(--text-muted)]">{t("vnc.loadingSession")}</p>
      </div>
    );
  }
  ...
  if (!tab || (!host && !sessionConnection)) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4">
        ...
        <p className="text-[var(--text-muted)]">{t("vnc.sessionNotFound")}</p>
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-6 px-6 py-6">
        ...
        {launchInfo && (
          <section className="rounded-2xl border border-[var(--border)] bg-[var(--bg-primary)] p-6">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <InfoCard label={t("vnc.launcher")} value={launchInfo.launcherName} />
              ...
              <InfoCard label={t("vnc.accessMode")} value={vncSettings.viewOnly ? t("vnc.viewOnlyEnabled") : t("vnc.viewOnlyDisabled")} />
            </div>
        ...
        {error && (
          <section className="rounded-2xl border border-[var(--danger)] bg-[var(--danger)]/10 p-6">
            <p className="text-sm font-medium text-[var(--danger)]">{t("vnc.errorTitle")}</p>
            <p className="mt-2 text-sm text-[var(--text-primary)]">{error}</p>
          </section>
        )}

        <section className="rounded-2xl border border-[var(--border)] bg-[var(--bg-primary)] p-6">
          <h2 className="text-sm font-semibold text-[var(--text-primary)]">{t("vnc.milestoneTitle")}</h2>
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <InfoCard label={t("vnc.milestones.host")} value={t("vnc.milestones.ready")} />
            <InfoCard label={t("vnc.milestones.launch")} value={t("vnc.milestones.ready")} />
          </div>
        </section>
