...
- **Sincronização automática** — push periódico em background com intervalo configurável (15 min, 30 min, 1 hora, 2 horas); notificação em caso de erro
...
- **Ordenação e filtros no Dashboard** — ordenar por nome (A→Z, Z→A) ou por último acesso (mais recente, mais antigo); filtro rápido por tags com chips clicáveis
- **Notificações nativas do SO** — alertas de sistema ao conectar/desconectar sessões SSH e ao concluir ou falhar uma sincronização
...
### Ordenar e filtrar hosts no Dashboard

**Ordenação:** à direita da barra de busca há um seletor de quatro botões de ícone:

| Ícone | Ordenação |
| --- | --- |
| A→Z | Nome alfabético crescente (padrão) |
| Z→A | Nome alfabético decrescente |
| Relógio ↓ | Último acesso mais recente primeiro |
| Relógio ↑ | Último acesso mais antigo primeiro (nunca conectados por último) |

**Filtro por tags:** quando hosts possuem tags cadastradas, uma linha de chips aparece abaixo dos filtros de grupo. Clicar em uma tag filtra os hosts que a contêm (lógica OR — qualquer tag selecionada inclui o host). Um botão "Limpar" remove os filtros de tag ativos.
...
Disponível na parte inferior da página de **Sincronização**, após configurar um provedor:

1. Ative o toggle **"Sincronização Automática"**
2. Escolha o intervalo: **15 min**, **30 min**, **1 hora** ou **2 horas**

O sync automático faz um **push silencioso** (sem notificação de sucesso) em background. Erros são notificados via notificação nativa do SO.

> **Atenção:** o sync automático não usa a senha mestra. Se você tiver "Sincronizar credenciais" ativado com senha mestra, as credenciais serão enviadas sem criptografia adicional no sync automático. Para sync seguro com credenciais, use o botão "Sincronizar Agora" manual.
...
│   ├── hooks/
│   │   └── useAutoSync.ts    # Hook de sync automático periódico (setInterval)
│   ├── pages/                # Páginas da aplicação
│   │   ├── Dashboard.tsx     # Grid de hosts com ordenação e filtros de tag
...
│   │   ├── Sync.tsx          # Sincronização remota + toggle de sync automático
...
│   │   ├── notifications.ts  # Notificações nativas do SO via tauri-plugin-notification
│   │   ├── sync.ts           # buildSyncPayload, parseSyncFile, applySyncPayload
│   │   ├── syncProviders.ts  # pushToProvider, pullFromProvider (providers de sync)
...
│   │   └── uiStore.ts        # Estado de UI (busca, grupo, tags, ordenação)
...
| 8 | ✅ Completo | Produtividade: snippets de comandos, perfis de túnel, workspaces, operações em lote, importação de `~/.ssh/config`, command palette (Ctrl+K) |
| 9 | ✅ Completo | UX avançada: ordenação e filtro por tags no Dashboard, notificações nativas do SO, sincronização automática periódica |
